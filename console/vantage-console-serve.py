    ...
    "provision-infratak": {
        ...
        "inputs": [
            {"name": "ref", "label": "infra-TAK version",
             ...
             "help": "a tag to pin, e.g. v10.1.55-alpha - leave empty for the pinned default"},
            {"name": "dry_run", "label": "Dry run", "pattern": r"^[01]$",
             "help": "1 = print every step without changing the box; run this first"},
        ],
...
def baseline_components(check_upstream=False):
    """The third-party baseline, for whoever asks: an operator, an auditor, or an agent.

    Read from `third-party.json`, which the cut generates by reading the versions out of the
    provisioner scripts themselves. The console does not run the generator: on an installed
    box the scripts are not laid out the way they are in the repo, so the fact is made where
    the sources are and shipped, exactly like VERSIONS.

    With check_upstream the release feeds are asked what they have published. That needs the
    network and still changes nothing: being behind is drift to report, and moving a pin is a
    deliberate edit by a person. A box with no route out gets 'could not ask' per component
    rather than a failure, because disconnected is a supported way to run this.
    """
    path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "third-party.json")
    if not os.path.isfile(path):
        return 1, ("this console carries no third-party baseline (third-party.json is "
                   "missing); it is generated by the cut, so a hand-assembled install may "
                   "not have it")
    try:
        rows = json.loads(open(path).read()).get("components") or []
    except Exception as exc:
        return 1, f"the third-party baseline could not be read: {exc}"
    ...
    for r in rows:
        ...
        if not feed:
            ...
            continue
        try:
            req = _rq.Request(feed, headers={"User-Agent": f"vantage-console/{VERSION}"})
            with _rq.urlopen(req, timeout=15) as fh:
                data = json.loads(fh.read().decode("utf-8", "replace"))
            latest = data[0].get("name") if isinstance(data, list) else data.get("tag_name")
            r["upstream"] = latest
            r["behind"] = bool(latest) and latest != r.get("version")
        except Exception as exc:
            r["upstream"] = None
            r["upstream_error"] = type(exc).__name__
    return 0, rows
...
def mcp_read_tools():
    return [
        ...
        {"name": "list_actions",
         ...
         "inputSchema": {"type": "object", "properties": {}}},
        {"name": "release_status",
         "description": "Where this estate stands against the published Vantage release: what "
                        "the publish surface carries, what this console runs, and which boxes "
                        "deviate from what the release pins. Reads only - it installs nothing "
                        "and touches no box. On a disconnected estate it reports that it could "
                        "not reach the publish surface, which is a fact, not an error.",
         "inputSchema": {"type": "object", "properties": {}}},
        {"name": "baseline_status",
         "description": "The third-party software this estate is built on: every component, "
                        "the version pinned, its licence and where to look it up. Pass "
                        "check_upstream to ask each project what it has released; that needs "
                        "the network and still changes nothing. Moving a pin is a deliberate "
                        "edit by a person, never a consequence of asking this.",
         "inputSchema": {"type": "object",
                         "properties": {"check_upstream": {"type": "boolean"}}}},
    ]
...
def mcp_write_tools(autonomy):
    ...
    # Staging is offered from `propose` upwards: it downloads the published archive and checks
    # its hash, installing nothing, so it is reversible by deleting a file.
    #
    # APPLYING is offered at `act` and nowhere else. The distinction is autonomy, not the task:
    # at `propose` an estate runs to the gate and a person presses go, which is the right
    # default and what the shipped role uses. An operator who sets `act` has decided their
    # agent may do deterministic work without asking each time, and applying a release IS
    # deterministic - vantage-apply-release verifies, snapshots, installs, waits for /healthz
    # and puts the old version back on its own if the console never answers. Refusing it at
    # every level would not be caution, it would be overriding a choice that is the operator's
    # to make.
    _release_stage = {
        "name": "release_stage",
        "description": "Download the published Vantage release onto this console's shelf and "
                       "verify its checksum. Staging is not applying: nothing is installed, no "
                       "box is touched, and the archive simply waits for a person to apply it "
                       "or carry it to an offline box. There is deliberately no tool to apply "
                       "a release; that stays with the operator.",
        "inputSchema": {"type": "object", "properties": {}}}
    if autonomy == "propose":
    ...
    if autonomy == "act":
        return [{"name": "run_action",
                ...
                _vault_build, _vault_write, _mission_pack, _release_stage,
                {"name": "release_apply",
                 "description": "Apply a staged release to this console. It verifies the "
                                "archive, snapshots what is installed, installs, restarts and "
                                "waits for the console to answer; if it never does, the "
                                "previous version is put back automatically. Offered only at "
                                "`act` autonomy. Say what you applied and what the verdict was.",
                 "inputSchema": {"type": "object",
                                 "properties": {"file": {"type": "string",
                                                         "description": "the staged archive; "
                                                                        "omit for the newest"}}}},
                {"name": "estate_update",
                 "description": "Push the checker this console carries to every enrolled box, "
                                "so the estate reports against what is actually released. "
                                "Idempotent and decision-free: a box either takes it or says "
                                "why. Consoles on boxes are never redeployed by this.",
                 "inputSchema": {"type": "object", "properties": {}}}]
    return []
...
def mcp_call_tool(conn, name, args, client):
    ...
    if name == "server_detail":
        ...
        # The declared loadout travels too. Without it an agent asked to cross-check open
        # ports against what the box is meant to run has to guess the mapping from typical
        # port usage, which is a guess presented as a check. Found by testing the security
        # review skill against a scenario: it did the right thing and said it was guessing,
        # which is how a missing tool shows up as an honest caveat instead of a wrong finding.
        return _mcp_text({"name": t.get("name"), "result": t.get("result"),
                          "checks": t.get("checks"), "software": software_rows(t),
                          "loadout": t.get("loadout")})
    ...
    if name == "release_status":
        # Composed from the same functions the Updates page uses, so an agent and an operator
        # never get different answers about where the estate stands.
        latest = check_vantage_release()
        manifest = fetch_release_manifest(latest["tag"]) if latest.get("ok") else None
        return _mcp_text({
            "console": VERSION,
            "release": VANTAGE_RELEASE,
            "published": latest,
            "deviation": (release_deviation(manifest, state, load_desired())
                          if manifest else []),
            "staged": sorted(f for f in (os.listdir(UPDATES_DIR)
                                        if os.path.isdir(UPDATES_DIR) else [])
                             if f.endswith(".tgz")),
            "note": "Read only. Nothing was downloaded, installed or changed. Staging is "
                    "release_stage; applying is release_apply at act autonomy, or a person "
                    "in the console.",
        })
    if name == "baseline_status":
        rc, rows = baseline_components(bool(args.get("check_upstream")))
        if rc:
            return _mcp_text({"error": rows})
        return _mcp_text({
            "components": rows,
            "note": "The versions here are read out of the provisioner scripts, so they are "
                    "what this release actually installs. Being behind upstream is drift to "
                    "report, not a fault: moving a pin is a deliberate edit by a person.",
        })
    if name == "release_stage" and conn.get("autonomy") in ("propose", "act"):
        code, res = start_release_pull({}, client)
        if code >= 400:
            return _mcp_text({"error": res.get("error", "could not stage the release")})
        out = dict(res)
        out["note"] = ("Staged only. Nothing is installed and no box was touched. A person "
                       "applies it in the console, or carries it to an offline box.")
        return _mcp_text(out)
    if name == "release_apply" and conn.get("autonomy") == "act":
        code, res = apply_release({"file": str(args.get("file", ""))}, client)
        if code >= 400:
            return _mcp_text({"error": res.get("error", "could not apply the release")})
        return _mcp_text(dict(res, note="Applying is detached: the console restarts. Read "
                                        "release_status afterwards for the verdict, and say "
                                        "plainly if it rolled back."))
    if name == "estate_update" and conn.get("autonomy") == "act":
        code, res = update_estate(client, "", True)
        if code >= 400:
            return _mcp_text({"error": res.get("error", "could not update the estate")})
