...
[[ -r "$SRC/../common/build-enrollment-dp.py" ]] && \
    install -m 0755 "$SRC/../common/build-enrollment-dp.py" /usr/local/bin/
install -m 0755 "$SRC/vantage-build-offline-bundle" /usr/local/bin/vantage-build-offline-bundle
# what this release is built on. Generated by the cut (the provisioner sources are not laid
...
# Stamp the release these artifacts came from. The console compares it against its own and
# says so when they differ, because a console can otherwise run a months-old provisioner
# while honestly reporting a current version - which it did, for weeks, unnoticed.
grep -m1 '^VANTAGE_RELEASE = ' "$SRC/vantage-console-serve.py" | cut -d\" -f2 \
    > "$VAR/artifacts/.release" 2>/dev/null || true
